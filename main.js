...
async function loadPlugin() {
      ...
      if (typeof plinfo["node_depends"] == "object") {
        for (var nid in plinfo["node_depends"]) {
          ...
          try {
            ...
            var exception = "";
            var success = false;
            for (moduleLoadTime = 1; moduleLoadTime <= 2; moduleLoadTime++) {
              wait.for.promise(new Promise(x => setTimeout(x, 20)));
              require.cache = {};
              try {
  ...
  global.commandMapping["uptime"] = {
    ...
    scope: function () {
      ...
      return {
        handler: "internal",
        data: `Uptime: ${(uptime / 3600 / 24).floor(0).pad(2)}:${utdate.getUTCHours().pad(2)}:${utdate.getUTCMinutes().pad(2)}:${utdate.getUTCSeconds().pad(2)}`
      };
    },
    compatibly: 0,
    handler: "INTERNAL"
  };
  global.commandMapping["upt"] = {
    args: {},
    desc: "Show uptime",
    scope: function () {
      var uptime = os.uptime();
  ...
  global.commandMapping["cpuarch"] = {
    ...
    scope: function () {
      ...
      return {
        handler: "internal",
        data: `System info:\r\n- CPU Arch: ${os.arch()}\r\n- x32: Support 3,5GB RAM only, but it better 40% x64, you know why it better, right? :))\r\n- x64: Uh, a lot of RAM, support x64 modules`
      };
    },
    compatibly: 0,
    handler: "INTERNAL"
  };
  global.commandMapping["os"] = {
    args: {},
    desc: "Show OS info",
    scope: function () {
      var uptime = os.uptime();
      ...
      return {
        handler: "internal",
        data: `- OS type: ${os.type()} (Platform: ${os.platform()})\r\n- OS version: ${os.release()}`
      };
    },
    compatibly: 0,
    handler: "INTERNAL"
  };
  global.commandMapping["memory"] = {
    args: {},
    ...
    scope: function () {
      ...
      return {
        handler: "internal",
        data: `- Total memory: ${os.totalmem() / 1048576} MB`
      };
    },
    compatibly: 0,
    handler: "INTERNAL"
  };
  global.commandMapping["heroku"] = {
    args: {},
    desc: "Check Heroku",
    scope: function () {
      var uptime = os.uptime();
      ...
      return {
        handler: "internal",
        data: `- Heroku: ${(!!process.env.PORT).toString()}`
      };
    },
    compatibly: 0,
    handler: "INTERNAL"
  ...
  global.commandMapping["help"] = {
    ...
    scope: function (type, data) {
      ...
      if (isNaN(parseInt(data.args[1])) && data.args.length != 1) {
        ...
        for (var i = 30 * (page - 1); i < 30 * (page - 1) + 30; i++) {
          if (i < hl.length) {
        ...
        if (type == "Discord") {
          ...
          var info1234 = ["[Bạn có biết?]: Bạn chưa biết gì cả.", "[Bạn có biết?]: Bạn đã biết.", "[Bạn có biết?]: Đừng bao giờ mong chờ điều gì ở C3CBot.", "[Bạn có biết?]: Việc spam lệnh nhiều có thể khiến bot chết.", "[Bạn có biết?]: Đây không phải là bot Mirai hay KB2A.", "[Bạn có biết?]: Đây là con bot tự viết code cho chính nó.", "[Bạn có biết?]: Đây là 1 lệnh vô dụng.", "[Bạn có biết?]: 228922 là một con số tuyệt vời.", "[Bạn có biết?]: 177013 là một con số tuyệt vời.", "[Bạn có biết?]: C3CBot không có khả năng hiểu được phụ nữ.", "[Bạn có biết?]: Acc phụ của admin là facebook.com/EXG.Ambush", "[Bạn có biết?]: Bot này không thể duy trì trạng thái 24/7 do quá nhiều box sử dụng và dẫn đến hạn chế tính năng.", "[Bạn có biết?]: Chủ bot là một người lười code và fix bot.", "[Bạn có biết?]: Bot này miễn phí và có thể thêm vào bất cứ box nào khác, nhưng việc box bị đánh dấu vào spam sẽ khiến admin không duyệt box.", "[Bạn có biết?]: Bot này đã được loại bỏ khá nhiều tính năng vì hạn chế tỉ lệ bị khoá mõm."];
          var info12345 = ["​"];
        mts += `\n(${getLang("PAGE", resolveID(type, data))} ${page}/${(hl.length / 30).ceil()})`;
        ...
        mts += `\n${info1234[Math.floor(Math.random() * info1234.length)]}`;
        return {
          handler: "internal",
          data: mts
        };
...
if (typeof global.data.cacheNameExpires != "object") {
  ...
  for (var n in global.data.cacheName) {
    global.data.cacheNameExpires[n] = currTime + 6048000000; //cacheName expires in 70 days.
  }
}
